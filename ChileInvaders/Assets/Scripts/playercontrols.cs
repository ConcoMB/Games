	...
	void  Update (){
		...
		if(Input.touchCount > 0){
			foreach(Touch touch2 in Input.touches) {
				FIXME_VAR_TYPE tapPosition= mainCamera.camera.ScreenToWorldPoint(touch2.position);
				tapPosition = tapPosition + Vector3(4,0,0);
				if(tapPosition.z > transform.position.z + 0.1f || tapPosition.z < transform.position.z - 0.1f){
				...
				if(twoAxis == true){
					if(tapPosition.x > transform.position.x + 0.1f || tapPosition.x < transform.position.x - 0.1f){
